...
namespace CrawlerPlus
{
    class Crawler
    {
        ...
        public void Start()
        {
            ...
            //委托的组播，在实现pageDownload这一委托时，将其中的方法用一个参数列表全部执行一遍
            ...
            while (tasks.Count < MaxPage)
            {
                ...
                Task task = Task.Run(() => DownloadAndParse(index,url));
                tasks.Add(task);
            }
        ...
        //将原先的执行任务制作成方法， 让task调用达到并行
        private void DownloadAndParse(int index, string url)
        {
             try
             {
